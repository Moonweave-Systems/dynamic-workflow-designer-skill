#!/usr/bin/env python3
"""V102 live-proof recorder and deterministic evidence contract tests."""
...
def resolve_manifest_out(value: str | Path) -> Path:
    ...
    candidate = raw if raw.is_absolute() else ROOT / raw
    resolved = candidate.resolve(strict=False)
    root = V102_OUT_ROOT.resolve(strict=False)
    try:
...
def run_manifest(manifest_path: Path, out_dir: Path) -> dict[str, Any]:
    ...
    prepare_owned_dir(out_dir, out_dir.name, source=manifest_path, root=V102_OUT_ROOT)
    records = []
    for fixture in fixtures:
        ...
        bundle = (
            fixture.get("bundle") if isinstance(fixture.get("bundle"), dict) else {}
        )
        errors = write_bundle(fixture_out, bundle)
        valid = not errors
        ...
        if (
            ...
            and bundle.get("decision") != expected_decision
        ):
            fixture_errors.append(
                f"expected decision {expected_decision}, got {bundle.get('decision')}"
            )
        records.append(
            {
                "id": fixture_id,
