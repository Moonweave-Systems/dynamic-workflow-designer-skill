...
def make_narrative(
    ...
    *,
    ...
    if roadmap_latest != "V93":
        add_blocker(blockers, "ERR_WORKFLOW_NARRATIVE_ROADMAP_STALE", "roadmap latest version is stale", surface="roadmap", actual=roadmap_latest, expected="V93")
    ...
    if activation_inputs.get("roadmap_latest_version") != "V93":
        add_blocker(blockers, "ERR_WORKFLOW_NARRATIVE_ACTIVATION_ROADMAP_STALE", "activation consumed a stale roadmap version", surface="activation", actual=activation_inputs.get("roadmap_latest_version"), expected="V93")
    ...
    stages = [
        stage(
            "chart",
            "Chart",
            "clear" if roadmap.get("decision") == "roadmap_reconciled" and roadmap_latest == "V93" and not roadmap.get("blocked_by") else "blocked",
...
def self_test() -> None:
    ...
    stale = dict(roadmap)
    stale["policy"] = {"latest_version": "V92"}
