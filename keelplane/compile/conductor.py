"""Conductor YAML emitter — translates a keelplane plan.json into Conductor workflow YAML.
            ...
            if not v:
                ...
                for item in v:
                    if isinstance(item, dict):
                        lines.append(f"{pad}  - {_yaml_dict(item, indent + 2).lstrip()}")
                    else:
...
def _build_phase_agents(
    ...
    workers = {w["id"]: w for w in plan.get("workers", []) if isinstance(w, dict) and w.get("id")}
    ...
    for i, phase in enumerate(phases):
        ...
        if worker and worker.get("prompt_contract", {}).get("inputs"):
            ...
            for inp in inputs:
                prompt_parts.append(f"- {inp.get('name', 'unknown')}: ${{{inp.get('name', 'value')}}}")
            prompt_parts.append("")
        ...
        prompt = "\n".join(prompt_parts) if prompt_parts else f"Execute {phase.get('name', phase_id)}."
        ...
        if worker:
            ...
            if schema:
                agent["output"] = {"result": {"type": "string", "description": str(schema)}}
...
def emit_yaml(plan: dict[str, Any]) -> str:
    ...
    plan_id = plan.get("plan_id", "keelplane-workflow")
    ...
    lines: list[str] = []
    lines.append("# Generated by keelplane compile --target conductor")
...
def run(args: argparse.Namespace) -> None:
    """Entry point for keelplane compile --target conductor."""
    if args.self_test:
        _self_test()
        return
...
def _self_test() -> None:
    """Run basic self-test."""
    print("keelplane compile (conductor) --self-test")
